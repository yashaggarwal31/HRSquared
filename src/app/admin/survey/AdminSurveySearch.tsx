...
//   console.log('dropdown val: ', dropwdownValue)
  ...
  useEffect(()=>{createFilterDate()},[dropwdownValue])
  ...
  const createFilterDate=()=>{
    ...
    switch(selectedDate){
        ...
        case DateSelect.Today:
            filterDate = new Date(currentDate.getFullYear(), currentDate.getMonth(), currentDate.getDate()-1).toISOString();
            setFilterISOString(filterDate);
            break;
        case DateSelect.OneWeek:
            filterDate = new Date(currentDate.getFullYear(), currentDate.getMonth(), currentDate.getDate()-7).toISOString(); ;
            setFilterISOString(filterDate);
            break;
        case DateSelect.OneMonth:
            filterDate = new Date(currentDate.getFullYear(), currentDate.getMonth()-1, currentDate.getDate()).toISOString();
            setFilterISOString(filterDate);
            break;
        case DateSelect.OneYear:
            filterDate = new Date(currentDate.getFullYear()-1, currentDate.getMonth(), currentDate.getDate()).toISOString();
            setFilterISOString(filterDate);
            break;
        default:
  ...
  return (
    <>
      ...
      <Input onChangeCallback={filterItems} />
        ...
        <label htmlFor="my-select" className="font-medium text-gray-700">Sort by:</label>
        <select id="my-select" className="w-[180px] px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" onChange={(event)=>handleOptionClick(event)}>
            ...
            <Link href={'survey/create-survey'} className="text-white bg-gradient-to-br from-green-400 to-blue-600 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-green-200 dark:focus:ring-green-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2">
                Create a new Survey
            </Link>
        </div>
